...
| Platform | File |
|---|---|
| Debian / Ubuntu | `ixd_1.0.3_amd64.deb` → `sudo dpkg -i ixd_1.0.3_amd64.deb` |
...
| Windows | `ixd-1.0.3-windows-x64.zip` → extract and run `ixd.exe`. See the note below on the SmartScreen warning; the **first** launch also takes a while, because Windows scans the whole folder before running anything from it |
...
| Host | Produces |
|---|---|
| Linux | `dist/ixd/`, `ixd_1.0.3_amd64.deb`, AppDir (`.AppImage` with `appimagetool`) |
