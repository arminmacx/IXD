...
|---|---|---|
| Windows | `ixd-1.0.19-windows-x64-setup.exe` | `ixd-1.0.19-windows-x64.zip` → extract, run `ixd.exe` |
| macOS (Apple silicon) | `ixd-1.0.19-macos-arm64.pkg`, or the `.dmg` | `ixd-macos-arm64.zip` |
| Debian / Ubuntu | `ixd_1.0.19_amd64.deb` → `sudo dpkg -i ixd_1.0.19_amd64.deb` | `ixd-linux-x86_64.tar.gz` → extract, run `ixd/ixd` |
...
| Host | Produces |
|---|---|
| Linux | `dist/ixd/`, `ixd_1.0.19_amd64.deb`, AppDir (`.AppImage` with `appimagetool`) |
